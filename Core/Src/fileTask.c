...
static RECV_STATUS_TypeDef transmittingStage(transmittingCtx_TypeDef* ctx) {
	UINT fnum = 0;
	static uint32_t packageNum = 0;		// 檔案接收次數計數器
...
static RECV_STATUS_TypeDef transmittingOverStage(transmittingCtx_TypeDef* ctx, void* _argument) {
	...
	// 清空佇列中剩餘的緩衝區指標並歸還到池中
	uartRxBuf_TypeDef *pRemainBuf = NULL;
	while (xQueueReceive(xFileQueue, &pRemainBuf, 0) == pdTRUE) {
		if (pRemainBuf != NULL) {
			xQueueSend(xFreeBufferQueue, &pRemainBuf, 0);
		}
	}
	
	// 關閉傳輸標誌必須在最後，確保所有緩衝區都已處理
